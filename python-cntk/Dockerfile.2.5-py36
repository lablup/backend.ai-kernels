# Microsoft CNTK 2.5 (CPU) & Python 3.6
# ref: https://docs.microsoft.com/en-us/cognitive-toolkit/Setup-Linux-Python

# mount from pre-builded common-cntk:2.5-py36 as wheel binary.
FROM lablup/common-cntk:2.5-py36 as cntk-binary
...
COPY --from=cntk-binary /tmp/cntk*.whl /tmp
...
RUN python -m ipykernel install --display-name "CNTK 2.5 on Python 3.6 (CPU-only)" && \
