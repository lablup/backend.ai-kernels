# Google Tensorflow 2.0 (CPU) & Python 3.6
# ref: https://tensorflow.org

# mount from pre-builded common-tensorfloe:2.0-py36 as wheel binary.
...
COPY --from=tensorflow-binary /tmp/*.whl /tmp
...
RUN python -m ipykernel install --display-name "TensorFlow 2.0 on Python 3.6 (CPU-only)" && \
